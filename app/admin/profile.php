...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'personal') {
        ...
        if ($name === '') { $errors[] = 'Name is required.'; }
            ...
            $conn->query("UPDATE users SET name='$name', phone='$phone', location='$location', location_name='$loc_name' WHERE id=$admin_id");
            $_SESSION['name'] = $name;
            $admin_name   = $name;
    ...
    if ($action === 'account') {
        ...
        if ($email === '' || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            if ($check->num_rows > 0) {
                ...
                $conn->query("UPDATE users SET email='$email' WHERE id=$admin_id");
    ...
    if ($action === 'avatar' && isset($_FILES['avatar'])) {
        ...
        if (!in_array($file['type'], $allowed)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $dest)) {
                ...
                $conn->query("UPDATE users SET image_paths='$sf' WHERE id=$admin_id");
...
$products_count  = $conn->query("SELECT COUNT(*) AS c FROM products WHERE farmer_id=$admin_id")->fetch_assoc()['c'];
$bulletins_count = $conn->query("SELECT COUNT(*) AS c FROM posts WHERE user_id=$admin_id")->fetch_assoc()['c'];
...
<html lang="en">
  <head>
    ...
    <title>FAIMS - Admin Profile</title>
        ...
        <div class="grid grid-cols-12 gap-4 sm:gap-5 lg:gap-12">
          ...
          <div class="card grow items-center p-4 sm:p-5">
            ...
            <div class="avatar size-20">
              <img class="rounded-full" :src="avatarPreview" alt="avatar">
              <div class="absolute right-0 m-1 size-4 rounded-full border-2 border-success bg-success dark:border-navy-700 dark:bg-accent"></div>
            </div>
            </template>
            <template x-if="!avatarPreview">
                                        ...
                                        <div class="absolute right-0 m-1 size-4 rounded-full border-2 border-success bg-primary dark:border-navy-700 dark:bg-accent"></div>
                                        <?php else: ?>
                                    ...
                                    <div class="avatar-overlay absolute right-0 m-12" onclick="document.getElementById('avatar-input').click()">
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.5"><path d="M3 13.5V15h1.5l8.5-8.5-1.5-1.5L3 13.5z"/><path d="M14.5 3.5l.5.5-1.5 1.5-.5-.5 1.5-1.5z"/></svg>
                                    </div>
